...
def delete_old_messages(state: AgentState, runtime: Runtime) -> dict[str, Any]|None:
    ...
    if len(messages) > 10:
        # remove the earliest 3 messages
...
agent = create_agent(
    model=llm,
    tools=tools,
    checkpointer=InMemorySaver(),  # Guardamos el estado del agente en memoria
    middleware=[delete_old_messages],  # Middleware para eliminar mensajes antiguos
